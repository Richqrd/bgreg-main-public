...
if __name__ == "__main__":
    ...
    subject = "jh101" # specify which subject to get channel name labels for
    ...
    batch = 1
    single_graph_embedding = [graph_embeddings[0][batch], graph_embeddings[1][batch]]
    # create labels corresponding to the batch
    targets = ground_truth[0]
    # labels = ['Preictal' if i==[0, 1, 0] else 'Nonictal' for i in targets]
    mapping = {
        (1, 0, 0): 'Preictal',
        (0, 1, 0): 'Ictal',
        (0, 0, 1): 'Nonictal'
    }
    labels = [mapping[tuple(arr)] for arr in targets]
    ...
    node_embeddings = embedding[batch, 1:3]

    # select only 1 iteration from a batch - NOTE: iteration is 2nd axis in node_embeddings
    iteration = 0
    single_node_embedding = [node_embeddings[0][iteration], node_embeddings[1][iteration]]

    # get channel names for the subject
    _, _, _, ch_names = get_gres(subject)
