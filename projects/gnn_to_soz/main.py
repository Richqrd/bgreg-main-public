...
from bgreg.data.analysis.graph.graph_representation import load_data, sequential_load_data, load_data_multi_subject
from bgreg.data.model.graph_neural_network.ecc_gat import Net as Net
from bgreg.data.model.graph_neural_network.gat import Net as Gat_Net
from bgreg.data.model.graph_neural_network.cnn_ecc_gat import Net as CNN_Net
from bgreg.data.model.graph_neural_network.log_reg import Net as Lognet
from bgreg.data.model.graph_neural_network.data import get_data_loaders, get_sequential_data_loader
...
from embedding_visualize import node_embedding, graph_embedding
...
def run(sub="ummc001", node_feat="ones", edge_feat="ones", adj="corr", normalize=True,
        ...
        self_loops=False, logdir=None, batch_size=32,
        fltrs_out=84, l2_reg=1e-3, dropout_rate=0.5, lr=0.001, epochs=2, es_patience=20, logreg=False,
        training=True, explain_all_data=False, gnn_explainer=False, folds=5, val_size=0.125):
    """
    ...
    set_random_seed(seed)  # sets tensorflow, numpy, python, and backend seed

    # At a computational cost, increases the determinism of tensorflow operations. Only works for CUDA machines
    ...
    if training:
        ...
        print(f'Running (not training) on {subject}')
    ...
    prev_dataset = [sub, "ID2", "ID3", "ID4", "ID5", "ID6"]
    et_dataset = ['ID1', 'ID2', 'ID3', 'ID4', 'ID5', 'ID6', 'ID7', 'ID8', 'ID9', 'ID10', 'ID11', 'ID12', 'ID13', 'ID14', 'ID15', 'ID16']
    ...
    data, preictal_size, ictal_size, postictal_size, test_sub = load_data_multi_subject(dataset, node_f=node_feat, edge_f=edge_feat,
                                                                          adj_type=adj, normalize=normalize,
                                                                          link_cutoff=link_cutoff,
                                                                          classification=classify,
                                                                          combined_features=combined_features,
    ...
    np.save('npy_test.npy', data)
    print('done')

    # k fold validation
    exp_logdir = logdir
    kf = KFold(n_splits=folds, shuffle=True, random_state=seed)
    for fold, (run_idx, test_idx) in enumerate(kf.split(data)):

        # split run_idx into training and validation, and shuffle the data
        np.random.shuffle(run_idx)
        val_idx = run_idx[0: int(len(run_idx) * val_size)]
        train_idx = run_idx[int(len(run_idx) * val_size):]

        np.random.shuffle(test_idx)

        # sample the data and create dataloaders
        ...
        # create subdirectories based on k folds
        logdir = Path(exp_logdir, f"fold-{fold}")
        if not os.path.exists(logdir):
            os.mkdir(logdir)
        ...
        if not logreg:
            ...
            model = Lognet(fltrs_out, l2_reg, dropout_rate, classify)
        optimizer = Adam(lr)
        if classify == "binary":
            model.compile(optimizer=optimizer,
                          loss="binary_crossentropy",
                          metrics=["accuracy", "AUC", f1_score, TruePositives(name="true_positives"),
                                   TrueNegatives(name="true_negatives"), FalsePositives(name="false_positives"),
            ...
            model.compile(optimizer=optimizer,
                          ...
                          metrics=["accuracy",
                                   "AUC",
                                   tfa.metrics.F1Score(average='macro', num_classes=3)])
    ...
    # Visualize embeddings FIXME
    # graph_embedding(logdir, model, test_loader, epoch=epochs, best_val_loss=best_val_loss, showPlot=False, subject=subject)

    # printing model weights
    if logreg:
        [weights, bias] = model.layers[4].get_weights()
        for i, weight in enumerate(weights):
            print(f'Weight sum for feature {i + 1} is {weight[0]}')
        print(f'Bias is {bias[0]}')

        json_logreg = [weights.tolist(), bias.tolist()]
        with open(os.path.join(logdir, "log_reg.json"), 'w') as f:
            json.dump(json_logreg, f)
    ...
    if gnn_explainer:
        ...
        # for sample in range(375,385,1):
            ...
            explainer = GNNExplainer(model, graph_level=True, verbose=False)
...
if __name__ == "__main__":
    ...
    # subjects = ['ummc002', 'ummc003', 'ummc004', 'ummc005', 'ummc006', 'ummc009', 'pt01', 'pt2', 'pt8']
    # excluded ['pt3', 'pt13', 'pt16'] bc analyses already run
    # subjects = ['umf001']
    # subjects = ['pt2', 'pt3', 'pt8', 'pt16', 'umf001']
    # subjects = ['pt8']
    ...
    for subject in subjects:
        ...
        # Run with ones features
        # run(sub=subject, combined_features=False, epochs=epochs)

        # Run with combined features
        # start_time = time.perf_counter()

        run(sub=subject, epochs=epochs, combined_features=True, node_feat="energy", edge_feat="coh",
            comb_edge_feat=['corr', 'coh_band_coh'], comb_node_feat=['energy', 'band_energy'],
