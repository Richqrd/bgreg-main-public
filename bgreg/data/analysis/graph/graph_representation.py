...
def graph_representation_elements(sub, w_sz=None, a_sz=None, w_st=0.125):
    ...
    """
    for run in get_runs(sub):
        print(f'Beginning run {run}')

        # obtain precomputed signals for the patient
        pp_preictal, pp_ictal, pp_postictal = get_preprocessed_data(sub, run)
...
def save_gres(signal_trace, run, sfreq, window_size, window_step, data_dir, trace_type, adj_window_size=20*1000):
    """
...
def load_data_multi_subject(subjects, node_f="ones", edge_f="corr", adj_type="ones", normalize=True,
              link_cutoff=0.1, classification="binary", combined_features=False,
              comb_node_feat=None, comb_edge_feat=None, self_loops=False, test_sub=[]):
    """
    Load preprocessed graph representations data from directory 'data/ds-003029-processed/preprocessed_graph'
    for any subject 'sub'
    This function creates the graph representations with their labeled values.
    ...
    For the multi-class classification problem of distinguishing between preictal, ictal, and postictal
    signal traces, graph representations are labeled with 0, 1, and 2, respectively.
    :param subjects: list of strings, [subject ID], i.e., "ummc001"
    :param node_f: string, node features. Valid entries: ones, energy, band_energy
    :param edge_f: string, edge features. Valid entries: ones, coh, corr, phase
    :param adj_type: string, adjacency matrix type. Valid entries:corr, coh, phase
    :param normalize: bool, operator to normalize data
    ...
    combined_graph_representations = []
    combined_preictal_size = 0
    combined_ictal_size = 0
    combined_postictal_size = 0
    num_nodes = []
    num_nf = []
    num_ef = []
    test_idx = []

    # get graph representations from each subject
    for sub in subjects:
        print(f'Loading data from {sub}')
        # or load_node_data
        graph_representations, ch_names, preictal_size, ictal_size, postictal_size = load_data(sub, node_f, edge_f, adj_type, normalize,
                                                                                               link_cutoff, classification, combined_features,
                                                                                               comb_node_feat, comb_edge_feat, self_loops)

        if sub in test_sub:
            new_idx = [i for i in range(len(combined_graph_representations), len(combined_graph_representations) + len(graph_representations))]
            test_idx.extend(new_idx)

        combined_graph_representations.extend(graph_representations)
        combined_preictal_size += preictal_size
        combined_ictal_size += ictal_size
        combined_postictal_size += postictal_size

        # get number of nodes, node features, and edge features for each subject's graph representation
        num_nodes.append(len(graph_representations[0][0][0]))
        num_nf.append(len(graph_representations[0][0][1][0]))
        num_ef.append(len(graph_representations[0][0][2][0][0]))


    # standardize all graph representations based on max number of nodes, and min number of node/edge features
    nodes = max(num_nodes)
    nfs = min(num_nf)
    efs = min(num_ef)
    ...
    for ind in range(len(combined_graph_representations)):
        extra_nodes = nodes - len(combined_graph_representations[ind][0][0])

        # padding the number of nodes for the adj matrix
        standardized_adj = np.pad(combined_graph_representations[ind][0][0], ((0, extra_nodes),
                                                                     (0, extra_nodes)))

        # padding the number of nodes for the labels
        standardized_labels = np.pad(combined_graph_representations[ind][1], (0, extra_nodes))

        # cutting the number of features
        standardized_nfs = combined_graph_representations[ind][0][1][:, 0:nfs]
        standardized_efs = combined_graph_representations[ind][0][2][:, :, 0:efs]

        # padding the node dimension for the features
        standardized_nfs = np.pad(standardized_nfs, ((0, extra_nodes),
                                                     (0, 0)))
        standardized_efs = np.pad(standardized_efs, ((0, extra_nodes),
                                                     (0, extra_nodes),
                                                     (0, 0)))

        # change the graph representation to the padded graph representation
        combined_graph_representations[ind][0] = [standardized_adj, standardized_nfs, standardized_efs]
        combined_graph_representations[ind][1] = standardized_labels


    return combined_graph_representations, combined_preictal_size, combined_ictal_size, combined_postictal_size, test_idx
...
def load_node_data(sub, node_f="ones", edge_f="corr", adj_type="ones", normalize=True,
    ...
    gre_preictal, gre_ictal, gre_postictal, ch_names = get_gres(sub)
    # print(ch_names)
    print(f"Loading graph reps from {sub}")
